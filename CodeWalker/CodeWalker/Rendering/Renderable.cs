...
namespace CodeWalker.Rendering
{
    ...
    public class Renderable : RenderableCacheItem<DrawableBase>
    {
        ...
        public bool EnableHairScale = false;
        ...
        public void UpdatePropTransform() 
        {
            ...
            switch (model.PedPropType)
            {
                ...
                case 2:
                case 3:
                    bone = bones.Where(x => x.Tag == 31086).FirstOrDefault(); //skel_head
                    break;
                case 4:
                    bone = bones.Where(x => x.Tag == 18905).FirstOrDefault(); //skel_l_hand
                    break;
                case 5:
                    bone = bones.Where(x => x.Tag == 57005).FirstOrDefault(); //skel_r_hand
                    break;
                case 6:
                    bone = bones.Where(x => x.Tag == 61007).FirstOrDefault(); //rb_l_forearmroll
                    break;
                case 7:
                    bone = bones.Where(x => x.Tag == 43810).FirstOrDefault(); //rb_r_forearmroll
                    break;
                case 8:
                    bone = bones.Where(x => x.Tag == 11816).FirstOrDefault(); //skel_pelvis
                    break;
                case 9:
                    bone = bones.Where(x => x.Tag == 14201).FirstOrDefault(); //skel_l_foot
                    break;
                case 10:
                    bone = bones.Where(x => x.Tag == 52301).FirstOrDefault(); //skel_r_foot
                    break;
                case 11:
                    bone = bones.Where(x => x.Tag == 60309).FirstOrDefault(); //ph_l_hand
                    break;
                case 12:
                    bone = bones.Where(x => x.Tag == 28422).FirstOrDefault(); //ph_r_hand
                    break;
                default:
                    break;
            }
            ...
            model.Transform = bone.AnimTransform;
        }
        ...
        private void UpdateAnim(ClipMapEntry cme)
        {
            ...
            {
                UpdateAnim(clipanim.Animation, clipanim.GetPlaybackTime(CurrentAnimTime, cme.PlaybackSpeed));
            }
            ...
            if (clipanimlist?.Animations != null)
            {
                foreach (var canim in clipanimlist.Animations)
                {
                    ...
                    UpdateAnim(canim.Animation, canim.GetPlaybackTime(CurrentAnimTime, cme.PlaybackSpeed));
                }
            }
        ...
        { 
            ...
            for (int i = 0; i < bones.Length; i++)
            {
                ...
                { continue; }
                if (bmodel.IsPedProp) //don't transform it here, do it later after all bones have been updated
                { continue; }
        ...
        private void UpdateAnimUV(ClipMapEntry cme, RenderableGeometry rgeom = null)
        {
            ...
            if (clipanim?.Animation != null)
            {
                UpdateAnimUV(clipanim.Animation, clipanim.GetPlaybackTime(CurrentAnimTime, cme.PlaybackSpeed), rgeom);
            }
            ...
            if (clipanimlist?.Animations != null)
            {
                ...
                {
                    if (canim?.Animation == null) continue;
                    UpdateAnimUV(canim.Animation, canim.GetPlaybackTime(CurrentAnimTime, cme.PlaybackSpeed), rgeom);
                }
            }
    ...
    public class RenderableModel
    {
        ...
        public void Init(DrawableModel dmodel)
        {
            ...
            for (int i = 0; i < geomcount; i++)
            {
                ...
                else if (Owner.Key is Drawable d && d.Name.StartsWith("p_"))
                {
                    IsPedProp = true;

                    var nameWithoutNumber = d.Name.Substring(0, d.Name.LastIndexOf('_'));
                    switch (nameWithoutNumber)
                    {
                        case "p_head": PedPropType = 0; break;
                        case "p_eyes": PedPropType = 1; break;
                        case "p_ears": PedPropType = 2; break;
                        case "p_mouth": PedPropType = 3; break;
                        case "p_lhand": PedPropType = 4; break;
                        case "p_rhand": PedPropType = 5; break;
                        case "p_lwrist": PedPropType = 6; break;
                        case "p_rwrist": PedPropType = 7; break;
                        case "p_hip": PedPropType = 8; break;
                        case "p_lfoot": PedPropType = 9; break;
                        case "p_rfoot": PedPropType = 10; break;
                        case "p_ph_l_hand": PedPropType = 11; break;
                        case "p_ph_r_hand": PedPropType = 12; break;
                        default: PedPropType = -1; break;
                    }
                }
            }
